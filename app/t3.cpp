...
void recv_data() {
  ...
  nn_connect(socket, "tcp://127.0.0.1:4445");
  while (running) {
    ...
    if (recv_size > 0) {
      ...
      memcpy(image_buff.data(), recv_msg+8, recv_size);
      cv.notify_one();
      nn_freemsg(recv_msg);
    }
...
int main(int argc, char *argv[]) {
  ...
  g_object_set(G_OBJECT(GstData.h264_encoder), "byte-stream", true, NULL);
  g_object_set(G_OBJECT(GstData.h264_encoder), "speed-preset", 1, NULL);
  g_object_set(G_OBJECT(GstData.h264_encoder), "bitrate", 10240, NULL);

  g_object_set(G_OBJECT(GstData.rtph264pay), "pt", 96, "config interval", 1, NULL);
  g_object_set(G_OBJECT(GstData.udpsink), "host", "192.168.0.102", "port", 5000, "sync", false, "async", FALSE, NULL);
  g_object_set(G_OBJECT(GstData.file_sink), "location", "222.mp4", NULL);

  //  g_object_set(G_OBJECT(GstData.app_source), "min-percent", 3, NULL);
  //  GstCaps *appsrc_caps;
  auto appsrc_caps = gst_caps_new_simple("video/x-raw", "format", G_TYPE_STRING, "BGR", "width", G_TYPE_INT, 1280,
                                         "height", G_TYPE_INT, 720, "framerate", GST_TYPE_FRACTION, 10, 1, NULL);
  ...
  gst_bin_add_many(GST_BIN(GstData.pipeline), GstData.app_source, GstData.video_convert, GstData.h264_encoder,
                   GstData.rtph264pay, GstData.udpsink, NULL);
  gst_element_link_many(GstData.app_source, GstData.video_convert, GstData.h264_encoder, GstData.rtph264pay,
                        GstData.udpsink, nullptr);
